...
const ChatSidebar = ({
  ...
  return (
    <div className="w-full md:w-80 bg-white border-r-2 border-slate-200 flex flex-col h-full">
      {/* Header */}
      <div className="p-4 md:p-6 border-b-2 border-slate-200 bg-gradient-to-r from-indigo-50 to-purple-50">
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-2">
        {sessions.length === 0 ? (
          <div className="text-center py-8 md:py-12 px-4">
            <div className="w-12 h-12 md:w-16 md:h-16 bg-gradient-to-r from-slate-100 to-slate-200 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <MessageSquare className="h-6 w-6 md:h-8 md:w-8 text-slate-400" />
            </div>
          ...
          sessions.map((session) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3 md:p-4 rounded-xl transition-all duration-300 border-2 group ${isActive
              ...
              >
                <div className="flex items-start gap-2 md:gap-3">
                  <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center flex-shrink-0 transition-all ${isActive
                    ? 'bg-gradient-to-r from-indigo-600 to-purple-600'
                    : 'bg-slate-100 group-hover:bg-slate-200'
                    }`}>
                    <MessageSquare className={`h-4 w-4 md:h-5 md:w-5 ${isActive ? 'text-white' : 'text-slate-600'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`font-semibold truncate mb-1 text-sm md:text-base ${isActive ? 'text-slate-800' : 'text-slate-700 group-hover:text-slate-900'
                      }`}>
                      {session.title}
                    </p>
      ...
      {/* Footer Tip */}
      {sessions.length > 0 && (
        <div className="p-3 md:p-4 border-t-2 border-slate-200 bg-gradient-to-r from-indigo-50 to-purple-50">
          <div className="flex items-start gap-2 md:gap-3">
            <div className="w-7 h-7 md:w-8 md:h-8 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg flex items-center justify-center flex-shrink-0">
